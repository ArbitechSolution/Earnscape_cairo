...
use crate::utils::{
    OWNER, USER1, USER2, ONE_TOKEN, HUNDRED_TOKENS, THOUSAND_TOKENS, MAX_LEVEL, DEFAULT_TAX,
    RESHUFFLE_TAX, BASIS_POINTS, LEVEL_1_COST, LEVEL_2_COST, LEVEL_3_COST, LEVEL_4_COST,
    LEVEL_5_COST, setup_earn_token, setup_stearn_token, setup_earnstark_manager, setup_staking,
    setup_vesting, transfer_token, approve_token, get_balance, setup_complete_system
};
...
trait IEarnscapeStaking<TContractState> {
    ...
    fn get_earn_staked_amount(self: @TContractState, user: ContractAddress, token: ContractAddress) -> u256;
    fn check_is_staked_with_earn(self: @TContractState, user: ContractAddress, token: ContractAddress) -> bool;
...
fn test_stake_with_earn_single_level() {
    ...
    let (earn_token, stearn_token, _, manager, staking_address, vesting_address, _) = 
    ...
    assert(
        staking.check_is_staked_with_earn(USER1(), earn_token), 
        'Should be staked with earn'
    );
    assert(
        staking.get_earn_staked_amount(USER1(), earn_token) == LEVEL_1_COST,
        'Wrong staked amount'
    );
}
...
fn test_stake_multiple_levels() {
    ...
    assert(
        staking.get_earn_staked_amount(USER1(), earn_token) == total_cost,
        'Wrong total staked'
    );
}
...
fn test_unstake() {
    ...
    assert(
        staking.get_earn_staked_amount(USER1(), earn_token) == 0,
...
fn test_get_user_data() {
    ...
    // Initially empty
    let (categories, levels, staked_amounts, staked_tokens) = staking.get_user_data(USER1());
    ...
    // Check data
    let (categories2, levels2, staked_amounts2, staked_tokens2) = staking.get_user_data(USER1());
